...
// Dependencies
const { Client, GatewayIntentBits, Partials, SlashCommandBuilder, PermissionFlagsBits, ChannelType, MessageFlags, REST, Routes, ActionRowBuilder, ButtonBuilder, ButtonStyle } = require("discord.js");
const {
  joinVoiceChannel,
  EndBehaviorType,
  getVoiceConnection,
  ...
  if (cookiesFile) {
    try {
      ...
      if (!content.includes("# HTTP Cookie File") || !content.includes(".youtube.com")) {
        ...
        logger.info("Valid cookies.txt found at " + cookiesFile);
      }
    } catch (e) {
      logger.warn("Failed to read cookies.txt: " + e.message);
    }
  } else {
    logger.warn("No YTDLP_COOKIES_FILE set. Public videos will work, but restricted videos may fail.");
  }
}
...
logger.info(`APP_ID=${process.env.APP_ID || process.env.APPLICATION_ID || "(unset)"}, DEV_GUILD_ID=${process.env.DEV_GUILD_ID || "(unset)"}`);
...
const voicesPaginations = new Map(); // key -> { pages, total, createdAt }
...
// -------------------- Helpers --------------------
function paginateTextBlocks(lines, header, footerBuilder, maxLen = 1800) {
  const pages = [];
  let buf = "";
  const push = () => { if (buf.trim().length) pages.push(buf.trim()); buf = ""; };
  ...
  return pages.map((content, idx) => {
    const footer = footerBuilder ? footerBuilder(idx + 1, pages.length) : "";
...
async function ytDlpExtractInfo(videoUrl, skipCookies = false) {
  return new Promise((resolve) => {
    ...
    // Only use cookies.txt if available and not skipping
    ...
    let out = "", err = "", loggedCookieError = false;
    let cp;
    try {
      ...
      cp.stderr.on("data", d => {
        ...
        if ((msg.includes("Could not copy Chrome cookie database") || msg.includes("The provided YouTube account cookies are no longer valid")) && !loggedCookieError) {
          logger.warn("[yt-dlp] Invalid or inaccessible cookies; ensure cookies.txt is valid or browser is closed. See https://github.com/yt-dlp/yt-dlp/wiki/Extractors#exporting-youtube-cookies");
          loggedCookieError = true;
        } else if (!msg.includes("Could not copy Chrome cookie database") && !msg.includes("The provided YouTube account cookies are no longer valid")) {
          err += msg + " \n";
          logger.info("[yt-dlp] " + msg.replace(/\n/g, " \\n "));
        }
      });
      cp.on("close", (code, signal) => {
        ...
        try {
          ...
          logger.warn("[yt] JSON parse failed: " + (e.message || e));
          resolve({ json: null, error: e.message });
        }
      });
      cp.on("error", e => {
        clearTimeout(timeout);
        logger.warn("[yt] yt-dlp JSON error: " + (e.message || e));
        resolve({ json: null, error: e.message });
      });
    } catch (e) {
      logger.warn("[yt] Failed to spawn yt-dlp JSON: " + (e.message || e));
      resolve({ json: null, error: e.message });
    }
  });
}
...
async function createAudioResourceFromYt(url, skipCookies = false) {
  ...
  let inputStream;
  let err = "", loggedCookieError = false;
  try {
    if (ytdlpExec) {
      ...
      cp.on("error", (e) => { err += e.message; logger.warn("yt-dlp error: " + (e.message || e)); });
      cp.stderr?.on("data", d => {
        const msg = String(d).trim();
        if ((msg.includes("Could not copy Chrome cookie database") || msg.includes("The provided YouTube account cookies are no longer valid")) && !loggedCookieError) {
          logger.warn("[yt-dlp] Invalid or inaccessible cookies; ensure cookies.txt is valid or browser is closed. See https://github.com/yt-dlp/yt-dlp/wiki/Extractors#exporting-youtube-cookies");
      ...
      cp.on("error", (e) => { err += e.message; logger.warn("yt-dlp spawn error: " + (e.message || e)); });
      cp.stderr?.on("data", d => {
        const msg = String(d).trim();
        if ((msg.includes("Could not copy Chrome cookie database") || msg.includes("The provided YouTube account cookies are no longer valid")) && !loggedCookieError) {
          logger.warn("[yt-dlp] Invalid or inaccessible cookies; ensure cookies.txt is valid or browser is closed. See https://github.com/yt-dlp/yt-dlp/wiki/Extractors#exporting-youtube-cookies");
    ...
    // removed: waiting for 'end' would exhaust the stream
    if (!hasData) {
      const errorMsg = err.includes("HTTP Error 403") 
        ...
        : err.includes("Could not copy Chrome cookie database") || err.includes("The provided YouTube account cookies are no longer valid")
        ? "Failed to play YouTube video: invalid or inaccessible cookies. Ensure cookies.txt is valid or browser is closed. See https://github.com/yt-dlp/yt-dlp/wiki/Extractors#exporting-youtube-cookies"
...
async function createAudioResourceFromYtWithFallback(url) {
  // Try with cookies.txt if available, otherwise skip cookies
  const cookiesFile = process.env.YTDLP_COOKIES_FILE;
  try {
    if (cookiesFile) {
      return await createAudioResourceFromYt(url, false); // Try with cookies.txt
    } else {
      logger.info("[yt] No cookies.txt set; attempting playback without cookies");
      return await createAudioResourceFromYt(url, true); // Skip cookies
    }
  } catch (e) {
    if (e.message.includes("HTTP 403") || e.message.includes("cookies")) {
...
// ---- Helper: idempotent guild command upsert (no clearing; single PUT; 429-aware) ----
function __slimCommands(arr) {
  try {
    return (arr || []).map(c => ({
      name: c.name,
...
async function upsertCommandsForGuild(rest, appId, guildId, commands) {
  let current = [];
  try { current = await rest.get(Routes.applicationGuildCommands(appId, guildId)); } catch {}
  const same = JSON.stringify(__slimCommands(current)) === JSON.stringify(__slimCommands(commands));
  if (same) return { skipped: true };
  try {
    await rest.put(Routes.applicationGuildCommands(appId, guildId), { body: commands });
    return { skipped: false };
  } catch (e) {
    if (e?.status === 429 && e?.rawError?.retry_after) {
      const ms = Math.ceil(e.rawError.retry_after * 1000) + 1500;
      logger?.warn?.(`[slash] 429 for guild ${guildId}; retrying in ${ms}ms`);
      await new Promise(r=>setTimeout(r, ms));
      await rest.put(Routes.applicationGuildCommands(appId, guildId), { body: commands });
      return { skipped: false, retried: true };
    }
    throw e;
  }
}
async function registerSlashCommands() {
  if (typeof __COMMANDS_REGISTERED__ !== "undefined" && __COMMANDS_REGISTERED__) {
    logger?.info?.("Slash commands already registered; skipping.");
    return;
  }

  // Build clean commands (no stray dots, valid choices usage)
  const commands = [
    ...
    new SlashCommandBuilder().setName("setchannel").setDescription("Set the text channel for logs (admin only)")
    ...
    new SlashCommandBuilder().setName("voice").setDescription("Set TTS voice")
    ...
    new SlashCommandBuilder().setName("voicepreset").setDescription("Pick a preset voice")
      .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
      .addStringOption(o => o
        .setName("preset")
        .setDescription("Preset key")
        .setRequired(true)
        .addChoices(...Object.entries(VOICE_PRESETS || {}).map(([k]) => ({ name: k, value: k })))),
    new SlashCommandBuilder().setName("rate").setDescription("Set TTS speaking rate")
      .addNumberOption(o => o.setName("value").setDescription("Rate").setRequired(true).setMinValue(0.25).setMaxValue(4.0)),
    new SlashCommandBuilder().setName("pitch").setDescription("Set TTS pitch")
      .addNumberOption(o => o.setName("value").setDescription("Pitch").setRequired(true).setMinValue(-20).setMaxValue(20)),
    new SlashCommandBuilder().setName("lang").setDescription("Set STT language code")
    ...
    new SlashCommandBuilder().setName("wake").setDescription("Enable/disable wake word")
    ...
    new SlashCommandBuilder().setName("say").setDescription("Speak custom text in the current voice channel")
      .addStringOption(o => o.setName("text").setDescription("What should I say?").setRequired(true)),
    new SlashCommandBuilder().setName("play").setDescription("Play audio from a URL or attachment in the current voice channel")
    ...
    new SlashCommandBuilder().setName("voices").setDescription("List available TTS voices")
      .addStringOption(o => o.setName("language").setDescription("Filter by language (e.g. en-US)").setRequired(false)),
    new SlashCommandBuilder().setName("persona").setDescription("View or set the AI persona used for responses")
      .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
      .addStringOption(o => o.setName("text").setDescription("New persona text (omit to view)").setRequired(false)),
    new SlashCommandBuilder().setName("stop").setDescription("Stop playback and clear the queue").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    new SlashCommandBuilder().setName("skip").setDescription("Skip the current track").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    new SlashCommandBuilder().setName("queue").setDescription("Show the current queue"),
    new SlashCommandBuilder().setName("register_here").setDescription("Admin: re-register slash commands to THIS server")
      .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
  ].map(c => c.toJSON());

  const appId  = process.env.APP_ID || process.env.APPLICATION_ID;
  const token  = process.env.DISCORD_TOKEN;
  const devGid = process.env.DEV_GUILD_ID;
  ...
  try {
    if (devGid) {
      await upsertCommandsForGuild(rest, appId, devGid, commands);
logger?.info?.(`Slash commands registered to DEV guild ${devGid}.`);
    }

    for (const g of client.guilds.cache.values()) {
      try {
        await upsertCommandsForGuild(rest, appId, g.id, commands);
logger?.info?.(`Slash commands registered to guild ${g.name} (${g.id}).`);
      } catch (e) {
        logger?.warn?.(`Slash registration failed for guild ${g?.name} (${g?.id}): ${e?.message}`);
      }
    }

    globalThis.__COMMANDS_REGISTERED__ = true;
  } catch (e) {
    logger?.warn?.("Bulk registration failed: " + (e?.message || e));
  }
}
...
client.on("messageCreate", async (msg) => {
  try {
    ...
    if (content !== "!!register") return;
    if (!isAdmin(msg.member)) {
      return msg.reply("Admins only.");
    }
    const appId = process.env.APP_ID || process.env.APPLICATION_ID;
    const token = process.env.DISCORD_TOKEN;
    if (!token || !appId) return msg.reply("Missing DISCORD_TOKEN or APP_ID; cannot register.");
    const rest3 = new REST({ version: "10" }).setToken(token);
    const commandsMsg = [
      new SlashCommandBuilder().setName("join").setDescription("Join your current voice channel"),
      new SlashCommandBuilder().setName("leave").setDescription("Leave the voice channel").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
      new SlashCommandBuilder().setName("setchannel").setDescription("Set the text channel for logs (admin only)").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).addChannelOption(o => o.setName("channel").setDescription("Text channel").setRequired(true).addChannelTypes(ChannelType.GuildText)),
      new SlashCommandBuilder().setName("voice").setDescription("Set TTS voice").addStringOption(o => o.setName("language").setDescription("Language (BCP-47, e.g., en-US)").setRequired(true)).addStringOption(o => o.setName("name").setDescription("Voice name (optional)").setRequired(false)),
      new SlashCommandBuilder().setName("voicepreset").setDescription("Pick a preset voice").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).addStringOption(o => o.setName("preset").setDescription("Preset key").setRequired(true).addChoices(...Object.entries(VOICE_PRESETS).map(([k, v]) => ({ name: k, value: k })) )),
      new SlashCommandBuilder().setName("rate").setDescription("Set TTS speaking rate").addNumberOption(o => o.setName("value").setDescription("Rate").setRequired(true).setMinValue(0.25).setMaxValue(4.0)),
      new SlashCommandBuilder().setName("pitch").setDescription("Set TTS pitch").addNumberOption(o => o.setName("value").setDescription("Pitch").setRequired(true).setMinValue(-20).setMaxValue(20)),
      new SlashCommandBuilder().setName("lang").setDescription("Set STT language code").addStringOption(o => o.setName("code").setDescription("Language code (BCP-47)").setRequired(true)),
      new SlashCommandBuilder().setName("settings").setDescription("Show current guild settings"),
      new SlashCommandBuilder().setName("wake").setDescription("Enable/disable wake word").addBooleanOption(o => o.setName("enabled").setDescription("Turn wake word on/off").setRequired(true)).addStringOption(o => o.setName("word").setDescription("Custom wake word").setRequired(false)),
      new SlashCommandBuilder().setName("say").setDescription("Speak custom text in the current voice channel").addStringOption(o => o.setName("text").setDescription("What should I say?").setRequired(true)),
      new SlashCommandBuilder().setName("play").setDescription("Play audio from a URL or attachment in the current voice channel").addStringOption(o => o.setName("url").setDescription("Direct or YouTube URL").setRequired(false)).addAttachmentOption(o => o.setName("attachment").setDescription("Upload an audio file").setRequired(false)),
      new SlashCommandBuilder().setName("voices").setDescription("List available TTS voices").addStringOption(o => o.setName("language").setDescription("Filter by language (e.g., en-US)").setRequired(false)),
      new SlashCommandBuilder().setName("persona").setDescription("View or set the AI persona used for responses").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).addStringOption(o => o.setName("text").setDescription("New persona text (omit to view)").setRequired(false)),
      new SlashCommandBuilder().setName("stop").setDescription("Stop playback and clear the queue").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
      new SlashCommandBuilder().setName("skip").setDescription("Skip the current track").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
      new SlashCommandBuilder().setName("queue").setDescription("Show the current queue"),
      new SlashCommandBuilder().setName("register_here").setDescription("Admin: re-register slash commands to THIS server").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    ].map(c => c.toJSON());
    const __resA = await upsertCommandsForGuild(rest3, appId, msg.guild.id, commandsMsg);
await msg.reply(__resA?.skipped ? "Commands already up to date." : "Re-registered commands to this server.");
} catch (e) {
    try { await msg.reply("Registration error: " + e.message); } catch {}
  }
});
...
case "register_here":
  ...
  try {
    ...
    const appId = process.env.APP_ID || process.env.APPLICATION_ID;
    const token = process.env.DISCORD_TOKEN;
    if (!token || !appId) { await i.editReply("Missing DISCORD_TOKEN or APP_ID; cannot register."); break; }
    const rest2 = new REST({ version: "10" }).setToken(token);
    // Rebuild the same commands list here
    const commandsLocal = [
      new SlashCommandBuilder().setName("join").setDescription("Join your current voice channel"),
      new SlashCommandBuilder().setName("leave").setDescription("Leave the voice channel").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
      new SlashCommandBuilder().setName("setchannel").setDescription("Set the text channel for logs (admin only)").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).addChannelOption(o => o.setName("channel").setDescription("Text channel").setRequired(true).addChannelTypes(ChannelType.GuildText)),
    ...
    const __resB = await upsertCommandsForGuild(rest2, appId, i.guildId, commandsLocal);
await i.editReply(__resB?.skipped ? "Commands already up to date." : "Re-registered commands to this server.");
} catch (e) {
    await i.editReply("Registration failed: " + e.message);
  }
  break;
      ...
      case "play":
        ...
        // Validate attachment MIME type / extension if provided
        if (!url && att) {
          const ct = (att.contentType || "").toLowerCase();
          const name = (att.name || "").toLowerCase();
          const allowedExt = /\.(mp3|ogg|oga|wav|m4a|aac)$/i;
          const isAudioMime = ct.startsWith("audio/");
          const isAllowedExt = allowedExt.test(name);
          if (!(isAudioMime || isAllowedExt)) {
            await replyEphemeral(i, "Attachment must be an audio file (mp3/ogg/wav/m4a/aac).");
            break;
          }
        }
    if (!url && !att) { await replyEphemeral(i, "Provide a URL (YouTube or direct audio) or attach a file."); break; }
      ...
      case "stop":
        ...
        if (!connStop) {
        if (!isAdmin(i.member)) { await replyEphemeral(i, "Admins only."); break; } await replyEphemeral(i, "I'm not in a voice channel."); break; }
        clearQueue(gid);
...
client.on("interactionCreate", async (i2) => {
  try {
    if (!i2.isButton()) return;
    const id = i2.customId || "";
    if (!id.startsWith("voices:")) return;
    const [key, action, idxStr] = id.split(":");
    const data = voicesPaginations.get(`${key}:${action}`) || voicesPaginations.get(key); // backward key lookup
    const parts = id.split(":");
    const baseKey = parts.slice(0, 3).join(":").replace(/:(prev|next|close):\d+$/, "");
    const store = voicesPaginations.get(baseKey);
    if (!store) { return i2.reply({ content: "This pagination has expired.", ephemeral: true }); }
    let idx = parseInt(parts.pop(), 10);
    if (isNaN(idx)) idx = 0;

    if (id.includes(":close:")) {
      voicesPaginations.delete(baseKey);
      return i2.update({ content: "Closed.", components: [] });
    }
    if (id.includes(":prev:")) idx = Math.max(0, idx - 1);
    if (id.includes(":next:")) idx = Math.min(store.pages.length - 1, idx + 1);

    const hasPrev = idx > 0;
    const hasNext = idx < store.pages.length - 1;
    const row = {
      type: 1,
      components: [
        { type: 2, style: ButtonStyle.Secondary, label: "Prev", custom_id: `${baseKey}:prev:${idx}`, disabled: !hasPrev },
        { type: 2, style: ButtonStyle.Secondary, label: "Next", custom_id: `${baseKey}:next:${idx}`, disabled: !hasNext },
        { type: 2, style: ButtonStyle.Danger, label: "Close", custom_id: `${baseKey}:close:${idx}` }
      ]
    };
    await i2.update({ content: store.pages[idx], components: [row] });
  } catch (e) {
    try { await i2.reply({ content: "Pagination error: " + e.message, ephemeral: true }); } catch {}
  }
});

// ===================== Text Command Fallbacks (bang-commands) =====================
// This block adds !commands that mirror your slash commands so the bot is usable
// even when application commands are not registered yet. It reuses your existing
// slash command handlers by emitting a fake Chat Input interaction.

if (!client.listenerCount("messageCreate")) {
  // Ensure we have message content intent enabled in Client init (you already do).
  client.on("messageCreate", async (msg) => {
    try {
      ...
      switch (cmd) {
        case "join":
          return client.emit("interactionCreate", __fakeInteraction(msg, "join", {}));

        case "leave":
          return client.emit("interactionCreate", __fakeInteraction(msg, "leave", {}));

        case "play": {
          // usage: !play <url> OR attach a file
          const url = args[0] || null;
          // NOTE: attachments are not mapped here; your slash handler reads "attachment" option.
          // If needed you can upload and then use /play with attachment manually.
          return client.emit("interactionCreate", __fakeInteraction(msg, "play", { url }));
        }

        case "stop":
          return client.emit("interactionCreate", __fakeInteraction(msg, "stop", {}));

        case "skip":
          return client.emit("interactionCreate", __fakeInteraction(msg, "skip", {}));

        case "queue":
          return client.emit("interactionCreate", __fakeInteraction(msg, "queue", {}));

        case "say":
          return client.emit("interactionCreate", __fakeInteraction(msg, "say", { text: args.join(" ") || "" }));

        case "settings":
          return client.emit("interactionCreate", __fakeInteraction(msg, "settings", {}));

        case "voice": {
          // usage: !voice <language> [name]
          const language = args[0] || "en-US";
          const name = args[1] || null;
          return client.emit("interactionCreate", __fakeInteraction(msg, "voice", { language, name }));
        }

        case "voicepreset": {
          // usage: !voicepreset <preset-key>
          const preset = args[0] || null;
          return client.emit("interactionCreate", __fakeInteraction(msg, "voicepreset", { preset }));
        }

        case "rate": {
          const value = parseFloat(args[0]);
          return client.emit("interactionCreate", __fakeInteraction(msg, "rate", { value }));
        }

        case "pitch": {
          const value = parseFloat(args[0]);
          return client.emit("interactionCreate", __fakeInteraction(msg, "pitch", { value }));
        }

        case "voices": {
          const language = args[0] || null;
          return client.emit("interactionCreate", __fakeInteraction(msg, "voices", { language }));
        }

        case "lang": {
          const code = args[0] || "en-US";
          return client.emit("interactionCreate", __fakeInteraction(msg, "lang", { code }));
        }

        case "wake": {
          // usage: !wake <true|false> [word]
          const enabled = String(args[0] || "").toLowerCase() === "true";
          const word = args.slice(1).join(" ") || null;
          return client.emit("interactionCreate", __fakeInteraction(msg, "wake", { enabled, word }));
        }

        case "persona": {
          const text = args.join(" ") || null;
          return client.emit("interactionCreate", __fakeInteraction(msg, "persona", { text }));
        }

        case "setchannel": {
          const ch = msg.mentions.channels.first();
          return client.emit("interactionCreate", __fakeInteraction(msg, "setchannel", { channel: ch?.id || null }));
        }

        case "register_here":
          return client.emit("interactionCreate", __fakeInteraction(msg, "register_here", {}));

        case "commands": // alias: show all available !commands
        case "help": {
          const lines = [
            "**Bang commands (text fallbacks):**",
            "!join, !leave, !play <url>, !stop, !skip, !queue,",
            "!say <text>, !settings, !voice <lang> [name], !voicepreset <key>,",
            "!rate <0.25-4.0>, !pitch <-20-20>, !voices [lang], !lang <code>,",
            "!wake <true|false> [word], !persona [text], !setchannel #channel, !register_here",
          ];
          return msg.reply(lines.join("\n"));
        }
      }
    } catch (err) {
      try { await msg.reply("Error: " + (err.message || String(err))); } catch {}
      if (typeof logger !== "undefined") logger.warn("bang-cmd error: " + (err.message || err));
    }
  });
}

// Fake a minimal Chat Input Interaction that your existing handlers already understand.
function __fakeInteraction(msg, name, dict) {
  const opt = {
    getString: (k) => (dict[k] ?? null),
    getNumber: (k) => (dict[k] ?? null),
    getBoolean: (k) => (dict[k] ?? null),
    getChannel: (k) => (dict[k] ? { id: dict[k] } : null),
  ...
  return {
    ...
    user: msg.author,
    options: opt,
    // Interaction reply shims:
    deferReply: async (opts) => { await msg.channel.sendTyping(); return; },
    reply: async (opts) => msg.reply(opts),
    editReply: async (opts) => msg.channel.send(opts),
    followUp: async (opts) => msg.channel.send(opts),
  };
}



// ===================== Interaction Guard Polyfill =====================
// Ensure downstream handlers never crash on i.isButton() etc.
// This runs FIRST and patches the interaction object in-place.
try {
  if (typeof client?.prependListener === "function") {
    client.prependListener("interactionCreate", (i) => {
      if (typeof i.isChatInputCommand !== "function") i.isChatInputCommand = () => false;
      if (typeof i.isButton !== "function") i.isButton = () => false;
      if (typeof i.isAnySelectMenu !== "function") i.isAnySelectMenu = () => false;
      if (typeof i.isStringSelectMenu !== "function") i.isStringSelectMenu = () => false;
      if (typeof i.isUserSelectMenu !== "function") i.isUserSelectMenu = () => false;
      if (typeof i.isRoleSelectMenu !== "function") i.isRoleSelectMenu = () => false;
      if (typeof i.isMentionableSelectMenu !== "function") i.isMentionableSelectMenu = () => false;
      if (typeof i.isChannelSelectMenu !== "function") i.isChannelSelectMenu = () => false;
      if (typeof i.isMessageComponent !== "function") i.isMessageComponent = () => false;
      if (typeof i.isContextMenuCommand !== "function") i.isContextMenuCommand = () => false;
      if (typeof i.isAutocomplete !== "function") i.isAutocomplete = () => false;
      if (typeof i.isModalSubmit !== "function") i.isModalSubmit = () => false;
    });
  }
} catch {}
