...
function loadSettings() { try { const raw = require(SETTINGS_FILE); for (const [gid,s] of Object.entries(raw)) guildSettings.set(gid,s);} catch(_){} }
async function saveSettings(){ try{ await fs.writeFile(SETTINGS_FILE,JSON.stringify(Object.fromEntries(guildSettings.entries()),null,2)); }catch(_){} }
loadSettings();
...
async function safeUnlink(f){try{await fs.unlink(f);}catch(_){}}
...
next=q.shift();guildQueues.set(gid,q);guildSpeaking.set(gid,true);playOggOpus(conn,next.buffer)}
