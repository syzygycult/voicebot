...
const guildCurrentTag = new Map(); // track what's playing (e.g., 'thinking')
...
function defaultGuildSettings() {
  return {
    ...
    logChannelId: null,
    trigger: { enabled: true, word: "hey bot", url: "https://sound-effects-media.bbcrewind.co.uk/mp3/07042083.mp3" }
  };
}
...
async function startThinkingSound(conn){
  try {
    ...
    const url = s.trigger?.url;
    if (!s.trigger?.enabled || !url) return;
    const res = await createAudioResourceFromUrl(url);
    enqueueResource(conn, res, "thinking");
  } catch (e) {
...
function interruptThinking(conn){
  try{
    const gid = conn.joinConfig.guildId;
    if (guildCurrentTag.get(gid) === "thinking") {
      const p = getOrCreateGuildPlayer(conn);
      p.stop(true); // immediate
      guildCurrentTag.delete(gid);
    }
...
function enqueueResource(conn, resource, tag = null){
  const gid = conn.joinConfig.guildId;
...
function handleGuildQueue(conn){
  const gid = conn.joinConfig.guildId;
  const q = guildQueues.get(gid) || [];
  if (q.length === 0){ guildSpeaking.set(gid,false); guildCurrentTag.delete(gid); return; }

  const next = q.shift();
  guildQueues.set(gid, q);
  guildSpeaking.set(gid, true);
  guildCurrentTag.set(gid, next.tag || null);
