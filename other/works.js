...
async function startThinkingSound(conn){
  try{
    ...
    const path = require("path");
    const prism = require("prism-media");
    const { demuxProbe, createAudioResource, StreamType, AudioPlayerStatus } = require("@discordjs/voice");
    ...
    if (fs.existsSync(oggPath)) {
      ...
      // Fallback: transcode local MP3 -> OGG/Opus with ffmpeg (requires ffmpeg in PATH)
