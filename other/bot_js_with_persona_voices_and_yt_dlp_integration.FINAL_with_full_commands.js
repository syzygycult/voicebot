...
function registerSlashCommands(){
  ...
  // Define the full command set (local to this function)
  const join = new SlashCommandBuilder().setName("join").setDescription("Join your current voice channel");
  const leave = new SlashCommandBuilder().setName("leave").setDescription("Leave the voice channel");
  ...
  const voicepreset = new SlashCommandBuilder()
  ...
  const rate = new SlashCommandBuilder()
    ...
    .addNumberOption(o => o.setName("value").setDescription("Rate").setRequired(true));
  const pitch = new SlashCommandBuilder()
    .setName("pitch").setDescription("Set TTS pitch")
    .addNumberOption(o => o.setName("value").setDescription("Pitch").setRequired(true));
  const lang = new SlashCommandBuilder()
  ...
  const stop = new SlashCommandBuilder().setName("stop").setDescription("Stop playback and clear the queue");
  const skip = new SlashCommandBuilder().setName("skip").setDescription("Skip the current track");
  const queue = new SlashCommandBuilder().setName("queue").setDescription("Show the current queue");

  const commands = [
    join.toJSON(), leave.toJSON(), setchannel.toJSON(), voice.toJSON(), voicepreset.toJSON(),
    rate.toJSON(), pitch.toJSON(), lang.toJSON(), settings.toJSON(), wake.toJSON(),
    say.toJSON(), play.toJSON(), voices.toJSON(), persona.toJSON(), stop.toJSON(), skip.toJSON(), queue.toJSON()
  ];
